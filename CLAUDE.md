...
- **Positive notifications (§3.21) — RESOLVED.** Governing rule: **a notification exists only to reach the user when they are not in the app.** Two v1 categories: *reminders* (invitations) and *re-engagement* (warm "we miss you", most constrained). **Celebrations cut from notifications** — every trigger fires while the app is open, so it duplicates the post-run summary / home screen; in-app only. Tips also in-app only. Opt-in asked during onboarding on screen 4 with a **mandatory in-app pre-prompt** before the OS prompt; single ask, per-category toggles in settings.
  - *Scheduling:* one reminder time (default 18:00, settings-editable, no day-picker) + fixed **anchor days** from the weekly commitment (2/wk → Tue, Sat; 3/wk → Mon, Wed, Sat). An anchor fires only if the week is incomplete and the user hasn't run that day. **HARD RULE: scheduling may read sessions-completed, never days-remaining** — the latter is a predatory notification in disguise.
  - *Caps:* reminders ≤ weekly commitment (and ≤1/day); re-engagement ≤2 per quiet period (at 2 and 4 quiet weeks, then silence). No global cap needed.
  - *Copy:* never name what was missed; reminders state the prescribed duration; no emoji. 3 rotating `session-invitation` variants, 1 fixed `still-here` string.
